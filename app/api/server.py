...
def start_server(bot_instance, host='0.0.0.0', port=5000):
    ...
    try:
        ...
        logger.info(f"API server initialized with bot instance")
        ...
        try:
            ...
            if "Address already in use" in str(e):
                ...
                try:
                    ...
                    logger.error(f"Failed to start API server on fallback port {port + 1}: {e_inner}")
            else:
                logger.error(f"Failed to start API server: {e}")
        except Exception as e:
            logger.error(f"An unexpected error occurred while starting the API server: {e}")
