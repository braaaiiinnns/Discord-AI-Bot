...
@ask_group.command(name="gpt", description="Ask GPT-4o-mini a question", guild=test_guild)
...
@ask_group.command(name="google", description="Ask Google GenAI a question", guild=test_guild)
...
@ask_group.command(name="claude", description="Ask Claude (as a poet) a question", guild=test_guild)
...
@ask_group.command(name="dall-e", description="Generate an image using DALL-E-3", guild=test_guild)
...
tree.add_command(ask_group, guild=test_guild)

@tree.command(name="clear_history", description="Clear your conversation history", guild=test_guild)
async def clear_history(interaction: discord.Interaction):
    ...
    # Log the cleared history
    logger.debug(f"Cleared history for user {interaction.user}: {user_state.get_context()}")
